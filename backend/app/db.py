...
def ensure_runtime_schema() -> None:
    with engine.begin() as connection:
        ...
        connection.execute(text("ALTER TABLE sessions ADD COLUMN IF NOT EXISTS platform TEXT"))
        connection.execute(text("ALTER TABLE sessions ADD COLUMN IF NOT EXISTS device_class TEXT"))
        connection.execute(text("ALTER TABLE sessions ALTER COLUMN device_class SET DEFAULT 'phone'"))
        connection.execute(text("UPDATE sessions SET device_class = 'phone' WHERE device_class IS NULL"))
        connection.execute(text("CREATE INDEX IF NOT EXISTS idx_sessions_workspace_device_class ON sessions(workspace_id, device_class, started_at DESC)"))
