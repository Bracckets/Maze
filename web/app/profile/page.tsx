      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--gap)" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap)" }}>
          <Card accent>
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
              ...
              <div>
                <div className="heading" style={{ fontSize: "1.15rem" }}>{user?.email ?? (locale === "ar" ? "تسجيل الدخول مطلوب" : "Sign in required")}</div>
                <div className="subtext" style={{ fontSize: "0.85rem" }}>{user?.workspace_name ?? (locale === "ar" ? "عضو في مساحة Maze" : "Maze workspace member")}</div>
              </div>
            </div>
          ...
          <Card>
            <div className="heading" style={{ marginBottom: 16 }}>{locale === "ar" ? "تفاصيل مساحة العمل" : "Workspace details"}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div style={{ background: "var(--surface-2)", borderRadius: "var(--r-md)", padding: "12px 14px", border: "1px solid var(--border)" }}>
                ...
                <p style={{ fontSize: "0.9rem", fontWeight: 600, color: "var(--text)", marginTop: 2 }}>{user?.workspace_name ?? (locale === "ar" ? "غير متاح" : "Unavailable")}</p>
              </div>
              <div style={{ background: "var(--surface-2)", borderRadius: "var(--r-md)", padding: "12px 14px", border: "1px solid var(--border)" }}>
                <p className="metric-label">{locale === "ar" ? "معرف مساحة العمل" : "Workspace ID"}</p>
                <p style={{ fontSize: "0.9rem", fontWeight: 600, color: "var(--text)", marginTop: 2 }}>{user?.workspace_id ?? (locale === "ar" ? "غير متاح" : "Unavailable")}</p>
              </div>
              <div style={{ background: "var(--surface-2)", borderRadius: "var(--r-md)", padding: "12px 14px", border: "1px solid var(--border)" }}>
        ...
        <Card>
          ...
          <p className="subtext" style={{ fontSize: "0.85rem", marginBottom: 22 }}>
            {locale === "ar" ? "حدّث البريد الإلكتروني واسم مساحة العمل المرتبطين بهذه المساحة." : "Update the account email and workspace name tied to this Maze workspace."}
          </p>
          {user ? (
